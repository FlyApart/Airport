...
public class PassengersController {
  ...
  @ApiImplicitParams({
    ...
    @ApiImplicitParam(
        ...
        paramType = "query",
        value =
            "Sorting criteria in the format: property(, "
                + "\"asc or desc\"). "
  ...
  public ResponseEntity<String> deletePassenger(@PathVariable String id) {
    Passengers passengers =
        (passengerRepository.findById(Long.valueOf(id)))
            .orElseThrow(() -> new EntityNotFoundException(Passengers.class, id));
  ...
  @Transactional
  public ResponseEntity<Passengers> createPassenger(
      @RequestBody @Valid PassengerSaveRequest passengerInfo) {
    Passengers passengers =
        passengersService.save(conversionService.convert(passengerInfo, Passengers.class));
    return new ResponseEntity<>(passengers, HttpStatus.CREATED);
  }

  @PutMapping(value = "/change")
  @Transactional
  public ResponseEntity<Passengers> updatePassenger(
      @RequestBody @Valid PassengerUpdateRequest passengerInfo) {
    return new ResponseEntity<>(
        passengersService.update(conversionService.convert(passengerInfo, Passengers.class)),
        HttpStatus.OK);
  }
}
