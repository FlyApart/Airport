...
public abstract class ConverterRequestFlights<S, T> extends EntityConverter<S, T> {
	...
	void validNumber (Class<?> sClass, Flights flights) {
		try {
			entityManager.createQuery ("from Flights f where f.fightsNumber=: title", Flights.class)
			             .setParameter ("title", flights.getFightsNumber ())
			             .getSingleResult ();
	...
	void validAirport (Class<?> sClass, Flights flights) {
		if (flights.getArriveAirport () == flights.getDepartureAirport ()) {
			throw new ConversionException (FlightsSaveRequest.class, Flights.class, flights, new ArgumentOfMethodNotValidException (Flights.class, "airport arrive" + flights.getArriveAirport () + " = " + "departure " + flights.getDepartureAirport ()));
		}
	}
	...
	void validDate (Class<?> sClass, Flights flights) {
		if (flights.getArriveDate ()
			...
			throw new ConversionException (FlightsSaveRequest.class, Flights.class, null, new ArgumentOfMethodNotValidException (Flights.class, "Date arrive" + flights.getArriveDate () + " <= " + "departure " + flights.getDepartureDate ()));
		}
	}
