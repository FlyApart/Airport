...
public class ConverterUpdateRequestPassports extends ConverterRequestPassports<PassportUpdateRequest, Passports> {
	...
	Passports findById (Passports passports, PassportUpdateRequest request) {
		...
		passports = ofNullable (entityManager.find (Passports.class, Long.valueOf (request.getId ()))).orElseThrow (() -> new ConversionException (PassportUpdateRequest.class, Passports.class, request, new EntityNotFoundException (Passports.class, request.getId ())));
		return passports;
	}
	...
	public Passports convert (PassportUpdateRequest request) {
		...
		if (request.getId () != null) {
			...
			if (request.getPassengerId () != null && !request.getPassengerId ()
			                                                 .equals (passports.getPassengersId ()
			                                                                   .getId ()
			                                                                   .toString ())) {
				throw new ConversionException (PassportUpdateRequest.class, Passports.class, request, new EntityNotFoundException ("id =" + request.getId () + " and passengerId = " + request.getPassengerId (), Passports.class));
			}
