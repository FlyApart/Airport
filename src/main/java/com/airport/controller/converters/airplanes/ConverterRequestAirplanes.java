...
public abstract class ConverterRequestAirplanes<S, T> extends EntityConverter<S, T> {
	...
	Countries findCountries (Class<?> sClass, String country) {
		...
		try {
			...
			throw new ConversionException (sClass, Airplanes.class, country, new EntityNotFoundException (" name = " + country, Countries.class));
		}
		return countries;
	}
	...
	void isUniqueModelAirplanes (Class<?> sClass, String model) {
		try {
			...
			throw new ConversionException (sClass, Airplanes.class, model, new ArgumentOfMethodNotValidException ("model = " + model));
		} catch (NoResultException e) {
			return;
		}
